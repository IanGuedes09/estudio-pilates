...
@section('title', 'Cadastrar Aluno - Studio Vitali')

@section('content')
    <div class="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
            <h1 class="text-3xl font-bold text-slate-900 dark:text-white">Cadastrar Novo Aluno</h1>
            <p class="text-sm text-slate-600 dark:text-slate-300">Este cadastro alimenta agenda, pagamentos e dashboard.</p>
        </div>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <select id="plano_aulas_semana" name="plano_aulas_semana" required
                        class="w-full rounded-xl border border-slate-300 bg-white/90 px-3 py-2 text-sm text-slate-700 dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-200">
                        <option value="2" @selected((int) old('plano_aulas_semana', 2) === 2)>2x por semana (R$ 200,00)</option>
                        <option value="3" @selected((int) old('plano_aulas_semana') === 3)>3x por semana (R$ 300,00)</option>
                    </select>
                ...
                <div>
                    ...
                    <select id="professor_id" name="professor_id"
                        ...
                        @foreach ($professores as $professor)
                            <option value="{{ $professor->id }}" @selected((string) old('professor_id') === (string) $professor->id)>
                                {{ $professor->nome }}
                            </option>
                        @endforeach
                    </select>
